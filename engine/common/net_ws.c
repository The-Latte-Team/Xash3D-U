...
static void NET_NetadrToSockadr( netadr_t *a, struct sockaddr_storage *s )
{
	...
	{	
		...
		{
			((struct sockaddr_in6 *)s)->sin6_family = AF_INET;
...
void NET_Config( qboolean multiplayer, qboolean changeport )
{
	...
	if( multiplayer )
	{
		...
		if( net.allow_ip6 )
			NET_OpenIP( changeport, net.ip6_sockets, net_ip6name.string, net_ip6hostport.value, net_ip6clientport.value, AF_INET );
