...
static void R_DrawTorus( vec3_t source, vec3_t delta, float width, float scale, float freq, float speed, int segments )
{
	...
	for( i = 0; i < segments; i++ )
	{
		...
		TriWorldToScreen2( point, screen );
...
static void R_DrawBeamFollow( BEAM *pbeam, float frametime )
{
	...
	if( !pnew && div != 0 )
	{
		...
		TriWorldToScreen2( pbeam->source, screenLast );
		TriWorldToScreen2( particles->org, screen );
	}
	else if( particles && particles->next )
	{
		VectorCopy( particles->org, delta );
		TriWorldToScreen2( particles->org, screenLast );
		TriWorldToScreen2( particles->next->org, screen );
...
static void R_BeamDraw( BEAM *pbeam, float frametime )
{
	...
	TriRenderMode2( FBitSet( pbeam->flags, FBEAM_SOLID ) ? kRenderNormal : kRenderTransAdd );

	if( !TriSpriteTexture2( model, (int)(pbeam->frame + pbeam->frameRate * gp_cl->time) % pbeam->frameCount ))
	{
		ClearBits( pbeam->flags, FBEAM_ISACTIVE );
		return;
	}
	...
	if( FBitSet( pbeam->flags, FBEAM_FADEIN ))
		TriColor4f2( pbeam->r, pbeam->g, pbeam->b, pbeam->t * pbeam->brightness );
	else if( FBitSet( pbeam->flags, FBEAM_FADEOUT ))
		TriColor4f2( pbeam->r, pbeam->g, pbeam->b, ( 1.0f - pbeam->t ) * pbeam->brightness );
	else TriColor4f2( pbeam->r, pbeam->g, pbeam->b, pbeam->brightness );
